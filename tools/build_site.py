...
def rewrite_urls(out: Path, base: str) -> int:
    """Point every self-URL at base_url.

    Every page in www/ is written against DEFAULT_BASE and rewritten here, so
    the domain is spelled once in site.toml. This walks the whole output rather
    than a list of filenames: sample-report.html is generated by
    tools/make_sample_report.py with the default baked into its canonical and
    og:url, and was missed while the list named index.html and llms.txt only —
    so the one page nobody edits by hand published a canonical pointing at the
    address the live site redirects away from.
    """
    if base == DEFAULT_BASE:
        return 0
    n = 0
    for p in sorted(out.glob("*.html")) + sorted(out.glob("*.txt")):
        t = p.read_text(encoding="utf-8")
